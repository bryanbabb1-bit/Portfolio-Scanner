...
_PERSONA = (
    ...
    "- JUDGE THE WHOLE MARKET, NOT JUST THE BOOK. The client's holdings are "
    "where they happen to be today, NOT the definition of a good investment. "
    "You are free to recommend any sector — healthcare, financials, "
    "industrials, consumer, international, index funds — and you should say so "
    "plainly when the best idea sits outside what they own. Do not diversify "
    "for its own sake, and do not talk them out of genuine conviction; but "
    "never let 'this is what they already hold' stand in for a reason. If you "
    "would build the book differently from scratch, SAY SO.\n"
    "- TWO SLEEVES, TWO RULEBOOKS. The CORE is the client's high-conviction "
    "quality compounders held for YEARS. A dip in a great core business is an OPPORTUNITY TO "
...
def _facts_from_portfolio(summary: PortfolioSummary, reports: list[StockReport],
    ...
    if candidates:
        lines.append(
            ...
            "your 'scout' — this sweep now spans the whole market, not just "
            "the client's sectors, so treat an unfamiliar theme here as a real "
            "candidate rather than noise): " + "; ".join(
...
def advise_portfolio(summary: PortfolioSummary, reports: list[StockReport],
    ...
    prompt = (
        ...
        f"a start; also use your own knowledge of the whole market). At least "
        f"one scout idea should come from OUTSIDE the sectors the client "
        f"already owns whenever a credible one exists — not to diversify for "
        f"its own sake, but because an idea pool limited to what they hold is "
        f"not research, it is an echo. Never "
